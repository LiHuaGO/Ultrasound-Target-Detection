...
class FullPAD_Tunnel(nn.Module):
    """
    An enhanced gated fusion module with adaptive feature alignment for the Full-Pipeline Aggregation-and-Distribution (FullPAD) paradigm.

    This improved version:
    1. Automatically handles size mismatches between input feature maps
    2. Includes optional channel adjustment when needed
    3. Maintains the original gated fusion functionality

    Args:
        adjust_channels (bool): Whether to add channel adjustment layers when channel dimensions differ. Default: True.
        gate_init (float): Initial value for the gating parameter. Default: 0.0 (start with original features).

    Methods:
        forward: Performs the gated fusion of two input feature maps with automatic size alignment.

    Examples:
        >>> import torch
        >>> # Case 1: Same size inputs
        ...
        >>> # Case 2: Different size inputs
        >>> model = FullPAD_Tunnel()
        >>> x1 = torch.randn(2, 64, 32, 32)
        >>> x2 = torch.randn(2, 128, 16, 16)  # Different channels and spatial dims
        >>> output = model([x1, x2])  # Automatically adjusted
        >>> print(output.shape)
        torch.Size([2, 64, 32, 32])
    """

    def __init__(self, adjust_channels=True, gate_init=0.0):
        super().__init__()
        self.gate = nn.Parameter(torch.tensor(gate_init))
        self.adjust_channels = adjust_channels

        # Channel adjustment layers (only created if needed)
        self.channel_adjust = None
        self.spatial_adjust = None

    def _prepare_adjust_layers(self, x1, x2):
        """Dynamically create adjustment layers if needed"""
        # Channel adjustment
        if x1.shape[1] != x2.shape[1] and self.adjust_channels:
            self.channel_adjust = nn.Sequential(
                nn.Conv2d(x2.shape[1], x1.shape[1], kernel_size=1),
                nn.BatchNorm2d(x1.shape[1]),
                nn.SiLU()
            ).to(x2.device)

        # Spatial adjustment (store scale factors)
        if x1.shape[2:] != x2.shape[2:]:
            self.spatial_scale = (x1.shape[2] / x2.shape[2], x1.shape[3] / x2.shape[3])

    def forward(self, x):
        if not isinstance(x, (list, tuple)) or len(x) != 2:
            raise ValueError(f"FullPAD_Tunnel expects 2 inputs, got {len(x) if isinstance(x, (list, tuple)) else 1}")

        x1, x2 = x

        # Automatic size adjustment
        if x1.shape != x2.shape:
            # Prepare adjustment layers if they don't exist
            if self.channel_adjust is None and self.spatial_adjust is None:
                self._prepare_adjust_layers(x1, x2)

            # Apply channel adjustment if needed
            if self.channel_adjust is not None:
                x2 = self.channel_adjust(x2)

            # Apply spatial adjustment if needed
            if hasattr(self, 'spatial_scale'):
                x2 = F.interpolate(x2,
