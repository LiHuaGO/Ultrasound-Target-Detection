...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir="", names=(), on_plot=None):
        ...
        seaborn.set_theme(font_scale=1.0 if nc < 50 else 0.8, rc=rc)  # for label size
...
def plot_pr_curve(px, py, ap, save_dir=Path("pr_curve.png"), names={}, on_plot=None):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ...
            df = pd.DataFrame(data={"recall": px, "precision": y})
            save_path = str(save_dir).replace("PR_cure.png", "") + names[i].replace("/", '') + ".csv"
...
def ap_per_class(
    ...
    if plot:
        ...
        plot_pr_curve(x, prec_values, ap, save_dir / f"{prefix}PR_curve.pdf", names, on_plot=on_plot)
    ...
    data_f1 = [{"class": names[i], "f1": f1[i]} for i in range(len(names))]
    pd.DataFrame(data_f1, columns=["class", "f1"]).to_excel(save_dir / f"{prefix}f1.xlsx", index=False)
